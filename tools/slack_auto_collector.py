...
输入你的数字分身的 Slack 姓名/用户名，自动：
  1. 搜索 Slack 用户，获取 user_id
  2. 找到与 Bot 共同的频道，拉取该用户发出的消息
  3. 输出统一格式，直接进 create-myskill 分析流程
...
CONFIG_PATH = Path.home() / ".my-skill" / "slack_config.json"
...
def collect_all(
    ...
    """采集某你的数字分身的所有 Slack 数据，输出到 output_dir"""
...
示例：
  ...
  python3 slack_auto_collector.py --name "john.doe" --output-dir ./knowledge/john --msg-limit 500
    ...
    parser.add_argument("--name", help="你的数字分身姓名或 Slack 用户名")
    parser.add_argument(
        "--output-dir",
        default=None,
