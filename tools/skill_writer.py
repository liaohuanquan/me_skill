...
用法：
    ...
    python3 skill_writer.py --action list --base-dir ./me
"""
...
def slugify(name: str) -> str:
    ...
    return slug if slug else "myskill"
...
def build_identity_string(meta: dict) -> str:
    ...
    identity = " ".join(parts) if parts else "你的数字分身"
...
def list_me(base_dir: Path) -> list:
    """列出所有已创建的专属数字分身（自己）Skill"""
    me = []
...
def main() -> None:
    ...
    parser.add_argument("--slug", help="你的数字分身 slug（用于目录名）")
    parser.add_argument("--name", help="你的数字分身姓名")
    ...
    parser.add_argument(
        "--base-dir",
        default="./me",
        help="专属数字分身（自己）Skill 根目录（默认：./me）",
    )
    ...
    if args.action == "list":
        me = list_me(base_dir)
        if not me:
            print("暂无已创建的专属数字分身（自己）Skill")
        else:
            print(f"已创建 {len(me)} 个专属数字分身（自己）Skill：\n")
            for c in me:
        ...
        slug = args.slug or slugify(meta.get("name", "myskill"))
