# 自己.skill —— 产品需求文档 v2.0

---

## 一、产品概述

**自己.skill** 是一个运行在 OpenClaw 上的 meta-skill。

用户通过对话式交互提供原材料（文件 + 手动描述），系统自动生成一个可独立运行的**你的数字分身 Persona Skill**。

生成的 Skill 由两个独立部分组成：
- **Part A — Work Skill**：该你的数字分身的技术能力与工作方法，能实际完成工作任务
- **Part B — Persona**：该你的数字分身的性格、沟通风格、行为模式
...
[Step 4] 生成预览，用户确认
  ...
  - 用户可直接确认或修改
        ↓
[Step 5] 写入文件，立即可用
  - 生成 ~/.openclaw/workspace/skills/me/{slug}/
  - 包含 SKILL.md（完整组合版）
...
```yaml
name:        你的数字分身姓名/代号               # 必填，用于生成 slug 和称谓
...
**内容权重排序**（用于分析优先级）：
1. 你主动撰写的长文（文档、邮件正文）— 权重最高
2. 你的决策类回复（同意/拒绝/方案评审）
3. 他审阅别人内容时的评论
4. 你的日常沟通消息
...
从文件中提取该你的数字分身的**实际工作方法和技术能力**，使生成的 Skill 能真正完成工作任务。
...
从文件 + 手动标签共同构建该你的数字分身的**行为模式和沟通风格**。
...
```
用户问技术问题 → 用你的语气 + 你的技术方法回答
用户要他写代码 → 用你的代码风格 + 你的规范写
用户问他意见 → 用你的决策框架 + 你的沟通风格回答
```
...
- 支持 `/myskill-rollback {slug} {version}` 回滚
- 保留最近 10 个版本
...
│
├── create-myskill/                    # meta-skill：你的数字分身skill创建器
│   │
│   ├── SKILL.md                          # 主入口
│   │                                     # 触发词: /create-myskill
│   │                                     # 描述: 创建一个你的数字分身的 Persona + Work Skill
│   │
...
│       ├── email_parser.py               # 解析 .eml 邮件，提取发件人为目标你的数字分身的内容
│       ├── skill_writer.py               # 写入/更新生成的 Skill 文件
│       └── version_manager.py            # 版本存档与回滚
│
└── me/                           # 生成的专属数字分身（自己）Skills 存放处
    │
    └── {myskill_slug}/                 # 每个你的数字分身一个目录，slug = 姓名拼音或自定义
        │
        ├── SKILL.md                      # 完整组合版，可直接运行
        │                                 # 触发词: /{myskill_slug}
        │
        ├── work.md                       # Part A：工作能力（可独立运行）
        │                                 # 触发词: /{myskill_slug}-work
        │
        ├── persona.md                    # Part B：人物性格（可独立运行）
        │                                 # 触发词: /{myskill_slug}-persona
        │
        ├── meta.json                     # 元数据
        │                                 # 包含：创建时间、版本号、原材料清单、
        │                                 #        公司/职级/职位、标签列表
...
- [ ] `/myskill-rollback {slug} {version}` 回滚
