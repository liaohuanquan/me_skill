...
提供你自己的工作原材料（飞书消息、钉钉文档、邮件、截图）加上你的主观描述<br>
生成一个**真正能替他工作的 AI Skill**<br>
用你的技术规范写代码，用你的语气回答问题，知道你什么时候会甩锅
...
> **April 4th Update：** 新增了两个示例你的数字分身——一个做安全的工程师和一个有趣的 HR，在 `me/` 目录下，欢迎体验！


## 支持的数据来源

> 目前还是自己.skill 的 beta 测试版本，后续会有更多来源支持，请多多关注！
...
按提示输入你的数字分身姓名、公司职级（如 `字节 2-1 算法工程师`）、性格标签，然后选择数据来源。所有字段均可跳过，仅凭描述也能生成。

完成后用 `/{slug}` 调用该专属数字分身（自己）Skill。

### 管理命令

| 命令 | 说明 |
|------|------|
| `/list-myskills` | 列出所有专属数字分身（自己）Skill |
| `/{slug}` | 调用完整 Skill（Persona + Work） |
| `/{slug}-work` | 仅工作能力 |
| `/{slug}-persona` | 仅人物性格 |
| `/myskill-rollback {slug} {version}` | 回滚到历史版本 |
| `/delete-myskill {slug}` | 删除 |
...
自己.skill ❯ 等等，这个接口的 impact 是什么？背景没说清楚。
...
自己.skill ❯ 上线时间对上了吗？那个需求改了好几个地方，还有其他变更。
```
...
每个专属数字分身（自己）Skill 由两部分组成，共同驱动输出：
...
运行逻辑：`接到任务 → Persona 判断态度 → Work Skill 执行 → 用你的语气输出`
...
├── me/           # 生成的专属数字分身（自己）Skill（gitignored）
├── docs/PRD.md
├── requirements.txt
└── LICENSE
```
...
>
> 我们写了一篇论文，详细介绍了自己.skill 的系统设计、两层架构（Work Skill + Persona）、多源数据采集、Skill 生成与进化机制，以及在真实场景中的评估结果。感兴趣的话可以看看！
