...
然后在 Claude Code 中说 `/create-myskill` 即可启动。

生成的专属数字分身（自己）Skill 默认写入 `./me/` 目录。
...
重启 OpenClaw session，说 `/create-myskill` 启动。
...
2. 选择 **From scratch**
3. 填写 App Name（如 `my-skill-bot`），选择目标 Workspace → **Create App**
...
> 提示：如果你不知道目标你的数字分身在哪些频道，可以先不邀请，运行采集时脚本会告知 Bot 加入了哪些频道，再补充邀请。
...
✅ 配置已保存到 /Users/you/.my-skill/slack_config.json
```
...
```bash
cd ~/.claude/skills/create-myskill   # 或你的项目 .claude/skills/create-myskill
...
# 列出已有专属数字分身（自己）Skill
python3 tools/skill_writer.py --action list --base-dir ./me
```
...
```
my-skill/        ← clone 到 .claude/skills/create-myskill/
├── SKILL.md            # skill 入口（官方 frontmatter）
├── prompts/            # 分析和生成的 Prompt 模板
├── tools/              # Python 工具脚本
├── docs/               # 文档（PRD 等）
│
└── me/         # 生成的专属数字分身（自己）Skill 存放处（.gitignore 排除）
    └── {slug}/
